...
- **Attachments:** click the paperclip badge on any task — or on a subtask, in its own row — to attach one or more images or PDFs, up to 12MB each. Each attachment is uploaded as its own file into an `attachments/` folder in your GitHub repo (tasks.json itself just keeps a small reference), so the list stays fast to load no matter how many files you attach. Both images and PDFs show a thumbnail once opened — for a PDF it's a preview of the first page — so you can tell attachments apart at a glance without opening each one; click a file's name to open the full file in a new tab. Removing an attachment deletes its file from the repo too.
