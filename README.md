...
- **Status:** click a task's (or subtask's) status control to cycle it through **Not started → In progress → Done → Not started**. In progress shows as a filled amber dot with a coloured edge on the row, so it's easy to spot at a glance without being counted as done anywhere (heading counts, Today/Week, Stats) — only Done counts as finished.
- Task and subtask titles (and notes) can hold more than one line — press **Shift+Enter** to add a line, plain **Enter** finishes editing same as before.
...
- Click the ▾ arrow on a heading or sub-heading to collapse it. Sub-headings start collapsed by default (a heading itself still starts expanded) — click the arrow to expand one, and it stays exactly as you left it from then on, on this device and after a reload. A sub-heading you create fresh via **+ Add sub-heading** starts expanded instead, so you can add a task to it right away.
...
- **Due-time reminders:** a task or subtask with a specific due *time* (not just a date) pops up a small reminder in the corner right around when it's due, with **+5m / +15m / +1h** snooze buttons or **Dismiss**. Because Daybook is a static site with no server of its own, this only works while the app is actually open somewhere — a browser tab, or the installed app running in the background on platforms that support that — it's not a true push notification that can wake your phone from nothing. Snoozing/dismissing is per-browser-session and resets on reload, same as the filters.
...
- The **This week** tab is a step back from Today: a fixed Monday–Sunday view of the current calendar week, one section per day (today's is highlighted), showing whatever's due that day across every heading. It's for a look-ahead across the week rather than just what needs attention right now — unlike Today, it doesn't show anything overdue from before this week, and completed tasks drop out of their day as soon as you check them off, same as everywhere else.
...
- **Reliability:** if you close the tab, refresh, or navigate away right in that one-second window, the pending change is flushed immediately rather than lost — you shouldn't need to wait for the "Saved to GitHub" indicator before it's safe to leave. If a save can't go through at all (e.g. you briefly lose your connection), the indicator switches to an error, the change is kept marked as unsaved so it's never silently treated as safe, and Daybook retries on its own in the background — refreshing or switching tabs while that's happening won't discard it either. If you ever do see the error indicator and want to force it, the **Sync now** button retries immediately. Switching away to another app or window and back (e.g. alt-tabbing) while you're mid-edit on a title or note no longer risks losing what you typed, either — Daybook now recognises you're still actively editing a field and won't refresh the list out from under it until you're done. This protection also covers the moment itself, not just the check beforehand: a background refresh (from switching back to the tab, hitting **Sync now**, or a Google Calendar sync) is a round-trip to the network, and if you start typing — say, adding a new task — right as one is in flight, it used to be able to land moments later and silently wipe what you'd just typed. That race is now closed: Daybook re-checks right before applying a refresh, and quietly skips it if you're still mid-edit, instead of overwriting the screen.
...
- If a save is rejected because the file changed underneath it (most often just this same tab or device having drifted to an older version after sitting open a while, rather than a genuinely different edit elsewhere), Daybook now re-fetches the current version and automatically retries that same save once before giving up — so a single stale save no longer costs you the change you just made (this used to be able to make an action like archiving a task silently revert a second or two later). Only if that retry also hits a conflict — a genuinely fast back-to-back edit from two places within a second or two of each other — does Daybook fall back to reloading the latest version and showing a banner asking you to redo the dropped change; there's still no way to automatically merge two truly simultaneous edits to the same file, but it's now a rare case rather than something a single stale tab could trigger on every save.
- **Outline (jump to heading):** the **Outline** button in the header, right next to **Theme**, opens a side menu sliding in from the left (below the header, which stays visible and usable the whole time), listing every heading at a glance — each with a live task count — so you can see the whole list's shape without scrolling through it. A heading with sub-headings gets its own ▸ chevron to reveal them right there in the panel — they start folded away each time you open it, so a heading with a dozen sub-headings doesn't swamp the overview; click the chevron to peek at them, click it again to fold them back. Click any heading or sub-heading itself (not the chevron) to scroll straight to that section, briefly highlighted so it's easy to spot; unlike most panels in the app, this one stays open afterward, so you can jump to several sections in a row without reopening it each time. Close it with its own **×** button, by clicking outside it, with Escape, or by clicking the Outline button again. It's read-only — opening it, folding a group open, or jumping around never changes anything about what's collapsed or saved.
- **View — Stacked, Tabs, or Status:** at the start of the row above the list, switch how headings are laid out. **Tabs** (the default) and **Stacked** show the same heading cards — every task grouped under its heading — the difference is just which one you land on: **Tabs** and **Status** default to showing one heading at a time, while **Stacked** shows every heading's card one after another. **Status** replaces the heading list with a kanban-style board — three columns, Not started / In progress / Done — grouping every task and subtask in view by where it stands, instead of by heading; a task's card lives in the column matching its own status, and a subtask at a different stage than its parent shows nested under a dimmed, title-only version of the parent card in whichever column matches the *subtask's* own status. Click a card's status dot right there on the board to cycle it, same as anywhere else; archived tasks stay out of Done, same as they're hidden everywhere else. A card sitting in the Done column also carries its own archive icon, so a finished task can go straight to the archive right from the board — no need to switch back to the List view first. All three share the same heading-filter bar above the list — **All** plus one button per heading, each showing its task count — so any of the three views can be scoped down to a single heading's tasks; which heading (or **All**) you've got selected carries over as you switch between the three, and resets to **All** each time you reload. This is purely a display preference (saved per-device) — it never changes what's saved.
- **Sort:** next to View, toggle between **Manual** (your own drag-and-drop order), **Priority** (High → Medium → Low → unflagged, due date as tiebreaker), and **Due date** (soonest first, undated tasks last). Switching back to Manual always restores your original order. This only ever reorders tasks within their heading — it never hides any; for that, see filtering below.
- **Notes & checklists:** click the notes icon on any task to jot free-text notes and/or add a checklist inside it — handy for a task with sub-steps. The notes box has a small formatting toolbar (**B** / *I* / U / bulleted list / numbered list / 🔗) for bold, italic, underline, lists, and hyperlinks — select some text and click a button, or place your cursor and click 🔗 to add a link (typing a bare domain like `example.com` automatically becomes `https://example.com`). The badge on the task shows a quick summary (e.g. "Notes • 2/3") without opening it. Once a task has notes or a checklist, that block shows up on its own from then on — no need to click the badge just to see what's already there — with a small chevron in its corner to fold it away again (and back) if you'd rather it stay out of the way; folding it never touches the content underneath.
- **Subtasks:** click the subtasks badge on any task, or its arrow, to break it into smaller steps, each with its own text, status (Not started/In progress/Done, same as a task), and optional due date — unlike checklist items, a subtask can be given its own date and shows as overdue in the same red styling as a regular task. The badge shows a "done/total" count (e.g. "1/3") and a small arrow that flips to show whether the panel is open. Once a task has at least one subtask, a second matching arrow appears right next to the task's own title, carrying its own small number showing exactly how many subtasks it has — so a task with steps nested inside it is impossible to miss at a glance, even buried in a sub-heading. Either arrow opens and closes the same panel. This is distinct from Notes & checklists (plain text steps with no dates) — use subtasks when the steps themselves have deadlines.
- **Heading & sub-heading notes and attachments:** a heading (or sub-heading) can hold any number of notes, right under its own bar — handy since headings often stand in for a whole project (hours agreed, a running list of follow-ups, a decision log). Unlike a task's notes, these don't hide behind a click — every note is shown right away, so there's nothing to open just to see what's there. Click **+ Add note** to add a new one; it opens straight into an editor with its own bold/italic/underline/bulleted-list/numbered-list/link toolbar (no checklist, since headings don't have one). Click **Done** — or just click elsewhere on the page — and the toolbar goes away entirely, leaving just the plain, rendered note (no editing chrome cluttering the view when you're not actively writing); click directly on that rendered text any time to reopen the editor and pick up where you left off. A small remove (✕) button sits beside each note, revealed on hover. The Attachments badge next to it works exactly like it does on a task — images or PDFs, up to 12MB, with paste-from-clipboard support — and stays a click-to-open panel, shown just below the notes. All of this is independent of any task inside that heading — a project-level "kickoff doc," an hours breakdown, or a scanned contract doesn't need to be attached to any one task to live there.
- **Attachments:** click the paperclip badge on any task — or on a subtask, in its own row — to attach one or more images or PDFs, up to 12MB each. Each attachment is uploaded as its own file into an `attachments/` folder in your GitHub repo (tasks.json itself just keeps a small reference), so the list stays fast to load no matter how many files you attach. Both images and PDFs show a thumbnail once opened — for a PDF it's a preview of the first page — so you can tell attachments apart at a glance without opening each one; click a file's name to open the full file in a new tab. With the attachments panel open, you can also just paste an image straight from your clipboard (Cmd/Ctrl+V) — handy for a screenshot — instead of saving it to a file first; it works for PNG, JPG, or whatever image format your clipboard hands over. Removing an attachment deletes its file from the repo too.
...
- **Archive:** once a task is checked off, an archive icon appears on it — archiving tucks it out of the main list without deleting it. Completed tasks also archive themselves automatically after two weeks, just to keep long-finished items from cluttering the list; nothing is ever deleted unless you delete it yourself. There are two ways to get back to an archived task: a "Show archived (N)" toggle right on its own heading (or sub-heading) reveals it in place, or the top-level **Archive** tab (next to Trash, showing a count once anything's archived) lists every archived task across the whole board in one place, each tagged with which heading or sub-heading it came from. Either way you can **Unarchive** it back to exactly where it was, or **Delete permanently**.
- **Recycle bin:** deleting a task from the list (the trash icon on the task itself, or "Delete permanently" from either archive view) doesn't erase it — a toast offers **Undo** for a few seconds, and even after that it keeps sitting in the **Trash** tab (which shows a count once anything's in it), tagged with which heading or sub-heading it came from. From there you can **Restore** it back to exactly where it was, or **Delete forever**, which is the one truly irreversible action — it asks you to confirm first, and doesn't go back into the bin.
...
- **Search:** the search box above the list matches everything about a task — its title, notes, tags, subtasks, and attachments (both filenames and, for images/PDFs, the text inside them — see below) — and combines with the Tags/Priority/Due date filters as an "and," same as they combine with each other. Like those filters, it's session-only and clears on reload. It also reaches beyond the task itself: a task shows up if the heading or sub-heading it lives under matches too — its title, any of its notes, or one of its own attachments — so searching a name or detail you only wrote in a heading's note still surfaces every task under that heading. And search tolerates small typos rather than requiring an exact match — searching "conferance" still finds "conference" — while a plain substring match (the old behavior) always still works too. If a match is sitting inside a collapsed heading or sub-heading, that section automatically expands so you can actually see it — the same applies to the Tags/Priority/Due date filters — without changing whether it's collapsed the next time you look (that's still whatever you last set it to).
