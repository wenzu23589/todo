...
- **View — Stacked, Tabs, or Status:** at the start of the row above the list, switch how headings are laid out. **Tabs** (the default) shows a row of tab buttons — one per heading, each showing its task count, plus an **All** tab — so you can jump straight to one heading and see only its tasks; **All** shows everything. **Stacked** is the classic look instead — every heading's card, one after another, no tab bar. **Status** replaces the heading list with a kanban-style board — three columns, Not started / In progress / Done — grouping every task and subtask in view by where it stands, instead of by heading; a task's card lives in the column matching its own status, and a subtask at a different stage than its parent shows nested under a dimmed, title-only version of the parent card in whichever column matches the *subtask's* own status. Click a card's status dot right there on the board to cycle it, same as anywhere else; archived tasks stay out of Done, same as they're hidden everywhere else. The heading-filter bar (the row of heading tabs, including **All**) is shared between **Tabs** and **Status** — so the board can be scoped down to one heading's tasks the same way the list can. This is purely a display preference (saved per-device); which heading tab you're on resets to **All** each time you reload.
