...
- **Outline (jump to heading):** the **Outline** button above the list opens a side menu sliding in from the left, listing every heading and sub-heading at a glance — each with a live task count — so you can see the whole list's shape without scrolling through it. Click any item to scroll straight to that section, briefly highlighted so it's easy to spot; unlike most panels in the app, this one stays open afterward, so you can jump to several sections in a row without reopening it each time. Close it with its own **×** button, by clicking outside it, with Escape, or by clicking the Outline button again. It's read-only — opening it or jumping around never changes anything about what's collapsed or saved.
