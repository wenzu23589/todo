...
- The **Status** tab shows every task and subtask as a kanban-style board with three columns — Not started, In progress, Done — so you can see everything at a given stage in one place instead of scanning the whole list. A task's card lives in the column matching its own status; a subtask can be at a different stage than its parent, so it shows nested under a dimmed, title-only version of the parent card in whichever column matches the *subtask's* own status — the parent's full, clickable card still only ever lives in the one column matching its own status. Click a card's status dot right there on the board to cycle it, same as in the List view; archived tasks stay out of Done, same as they're hidden everywhere else.
...
- **View — Stacked or Tabs:** right next to Sort, switch how headings are laid out in the List view. **Stacked** (the default) is the classic look — every heading's card, one after another, scroll through them all. **Tabs** replaces that with a row of tab buttons — one per heading, each showing its task count, plus an **All** tab — so you can jump straight to one heading and see only its tasks, without the rest of the list in the way; **All** shows everything stacked, same as before. This is purely a display preference (saved per-device, like Sort) — it never changes what's saved, and which tab you're on resets to **All** each time you reload.
