...
- **Heading & sub-heading notes and attachments:** a heading (or sub-heading) gets the same Notes and Attachments badges as a task, right in its own bar — handy since headings often stand in for a whole project. Notes there support the same bold/italic/underline/link formatting as task notes (no checklist, since headings don't have one), and Attachments works exactly like it does on a task — images or PDFs, up to 12MB, with paste-from-clipboard support. It's all independent of any task inside that heading — a project-level "kickoff doc" or a scanned contract doesn't need to be attached to any one task to live there.
