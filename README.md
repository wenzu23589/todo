...
- Click a task's due-date pill to give it a date (and optional time). Overdue tasks are flagged in red. If the task already has a due date, the same pill also offers **Snooze** shortcuts — **+1 day** or **+1 week** — to push it back without opening the full date picker.
...
- **Subtasks:** click the subtasks icon on any task to break it into smaller steps, each with its own text, checkbox, and optional due date — unlike checklist items, a subtask can be given its own date and shows as overdue in the same red styling as a regular task. The badge shows a "done/total" count (e.g. "1/3") at a glance. This is distinct from Notes & checklists (plain text steps with no dates) — use subtasks when the steps themselves have deadlines.
