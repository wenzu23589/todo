...
- If two devices save at almost the same moment, the second save detects the conflict, reloads the latest version from GitHub, and shows a banner — just redo the change that got dropped. This is the one case where a change can genuinely still be lost, since there's no way to merge two edits to the same file automatically — it's rare unless you're actively editing from two places within a second or two of each other.
