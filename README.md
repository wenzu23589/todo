...
- **Subtasks:** click the subtasks badge on any task (or its arrow) to break it into smaller steps, each with its own text, checkbox, and optional due date — unlike checklist items, a subtask can be given its own date and shows as overdue in the same red styling as a regular task. The badge shows a "done/total" count (e.g. "1/3") and a small arrow that flips to show whether the panel is open, so it's easy to tell at a glance that it's an expand/collapse control. This is distinct from Notes & checklists (plain text steps with no dates) — use subtasks when the steps themselves have deadlines.
